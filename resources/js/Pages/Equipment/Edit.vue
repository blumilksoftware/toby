...
const form = useForm(`EditEquipmentItem:${props.equipmentItem.id}`, {
  idNumber: props.equipmentItem.idNumber,
  name: props.equipmentItem.name,
  labels: props.equipmentItem.labels,
  isMobile: props.equipmentItem.isMobile,
  assignee: props.equipmentItem.assignee,
  assignedAt: props.equipmentItem.assignedAt,
})

function editEquipmentItem() {
  form
    .transform(data => ({
      ...data,
      assignee: data.assignee?.id,
    }))
    .put(`/equipment-items/${props.equipmentItem.id}`)
}
</script>

<template>
  <InertiaHead title="Edytowanie sprzętu" />
  <div class="mx-auto w-full max-w-7xl bg-white shadow-md">
    ...
    <form
      ...
      @submit.prevent="editEquipmentItem"
    >
      ...
      >
        ...
        <div class="mt-2 sm:col-span-2 sm:mt-0">
          <Switch
            ...
            :class="[form.isMobile ? 'bg-blumilk-500' : 'bg-gray-200', 'relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blumilk-500']"
          >
            <span
              :class="[form.isMobile ? 'translate-x-5' : 'translate-x-0', 'pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200']"
            />
          </Switch>
        </div>
      </div>
